...
abstract class HttpMessage {
  ...
  /// 按 Content-Encoding 解压后的 body 字节（gzip / deflate / br）。
  /// Hex、Text、预览使用；解压失败或无需解压时回退原始字节。
  Uint8List get decodedBodyBytes {
    final raw = _body;
    if (raw == null || raw.isEmpty) return Uint8List(0);
    try {
      if (headers.isGzip) return Uint8List.fromList(gzipDecode(raw));
      final encoding = headers.contentEncoding;
      if (encoding == 'deflate') return Uint8List.fromList(zlibDecode(raw));
      if (encoding == 'br') return Uint8List.fromList(brDecode(raw));
    } catch (_) {
  ...
  /// 不做 Content-Encoding 解码的原始 body 字符串，Raw 视图专用
  String get rawBodyAsString {
    final raw = _body;
    if (raw == null || raw.isEmpty) return '';
    try {
      if (charset == 'utf-8' || charset == 'utf8') return utf8.decode(raw);
