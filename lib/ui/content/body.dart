...
class HttpBodyWidget extends StatefulWidget {
  ...
  final bool showToolbar; // 是否显示标题栏和 Tab 切换（HttpCanary 紧凑模式用）
  final ViewType? viewType; // 强制指定视图类型（紧凑模式用）
...
class HttpBodyState extends State<HttpBodyWidget> {
  ...
  /// 根据内容类型选择最适合的预览视图
  static ViewType _previewViewType(HttpMessage message) {
    final ct = message.contentType;
    if (ct == ContentType.image) return ViewType.image;
    if (ct == ContentType.video) return ViewType.video;
    if (ct == ContentType.json) return ViewType.jsonText;
    if (ct == ContentType.html || ct == ContentType.xml || ct == ContentType.js || ct == ContentType.css) {
  ...
  Widget build(BuildContext context) {
    ...
    // HttpCanary 紧凑模式：不渲染标题栏和 Tab，仅渲染指定视图类型的 body
    if (!widget.showToolbar) {
      var viewType = widget.viewType ?? ViewType.text;
      if (viewType == ViewType.preview) {
        viewType = _previewViewType(widget.httpMessage!);
      }
      return _Body(
...
class _BodyState extends State<_Body> {
  ...
  Future<String?> getBody() async {
    ...
    if (viewType == ViewType.hex) {
      return currentMessage.decodedBodyBytes.map(intToHex).join(" ");
    }
  ...
  Widget _getBody(ViewType type) {
    ...
    if (type == ViewType.image) {
      return Center(child: Image.memory(message.decodedBodyBytes, fit: BoxFit.scaleDown));
    }
    if (type == ViewType.video) {
      return const Center(child: Text("video not support preview"));
    }
    if (type == ViewType.hex) {
      return HexViewer(data: message.decodedBodyBytes, searchController: widget.searchController);
    }

    // Raw：保留原始字节（不做 Content-Encoding 解码），与原版一致
    if (type == ViewType.raw) {
      return HighlightTextWidget(
          text: message.rawBodyAsString,
...
/// 三栏 Hex 视图：偏移列 | 字节码列 | ASCII 列（与原版 HttpCanary 一致）
/// 按行构建 + 惰性列表，避免大响应一次性生成百万级 TextSpan 导致卡死。
class HexViewer extends StatelessWidget {
  ...
  static const TextStyle _base = TextStyle(fontFamily: 'monospace', fontSize: 12, height: 1.5);
  static const Color _offsetColor = Color(0xFF9E9E9E);
  static const Color _byteColor = Color(0xFF1976D2);
  static const Color _asciiColor = Color(0xFF388E3C);

  @override
  Widget build(BuildContext context) {
    final lineCount = (data.length / bytesPerRow).ceil();

    // 横向滚动内容宽度：用一整行样本测量，避免不同设备字宽下被裁剪
    final sampleText = '${'0' * 8}  ${'ff ' * bytesPerRow} '
        ' |${'#' * bytesPerRow}|';
    final tp = TextPainter(
        text: TextSpan(text: sampleText, style: _base), textDirection: TextDirection.ltr)
      ..layout();
    final lineWidth = tp.width + 24;
  ...
  Widget _buildLine(int index) {
    ...
    final end = (start + bytesPerRow).clamp(0, data.length);
    final chunk = data.sublist(start, end);

    // 偏移列（灰）
    final offset = TextSpan(
        text: '${start.toRadixString(16).padLeft(8, '0')}  ',
        style: _base.copyWith(color: _offsetColor));

    // 字节码列（蓝），第 8 字节后多一个空格
    final hexBuffer = StringBuffer();
    for (var j = 0; j < bytesPerRow; j++) {
      if (j < chunk.length) {
        hexBuffer.write('${chunk[j].toRadixString(16).padLeft(2, '0')} ');
      } else {
        hexBuffer.write('   ');
      }
      if (j == 7) hexBuffer.write(' ');
    }
    final hex = TextSpan(text: hexBuffer.toString(), style: _base.copyWith(color: _byteColor));

    // ASCII 列（绿）
    final asciiBuffer = StringBuffer()..write(' |');
    for (final b in chunk) {
      asciiBuffer.write((b >= 32 && b < 127) ? String.fromCharCode(b) : '.');
    }
    asciiBuffer.write('|');
    final ascii = TextSpan(text: asciiBuffer.toString(), style: _base.copyWith(color: _asciiColor));

    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 12),
      child: Text.rich(TextSpan(children: [offset, hex, ascii]), softWrap: false),
    );
  }
}
